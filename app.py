            ...
            try:
                if df_today is not None and not df_today.empty:
                    if 'ML_Prob' not in df_today.columns:
                        ...
                        saved_systems = {}
                        if pool_choice in ["Public", "Combined"] and os.path.exists("saved_smart_systems.json"):
                            with open("saved_smart_systems.json", "r") as f:
                                try: saved_systems.update(json.load(f))
                                except: pass
                                
                        if pool_choice in ["Admin Secret", "Combined"] and os.path.exists("admin_systems.json"):
                            with open("admin_systems.json", "r") as f:
                                try: saved_systems.update(json.load(f))
                                except: pass
    ...
    with tab4:
        ...
        if df_all is not None and not df_all.empty:
            ...
            if st.session_state.get("is_admin"):
                ...
                with c_pub:
                    if os.path.exists("saved_smart_systems.json"):
                        with st.expander("📖 View Active PUBLIC Systems"):
                            try:
                                with open("saved_smart_systems.json", "r") as f: saved_dict = json.load(f)
                                if saved_dict:
                ...
                with c_sec:
                    if os.path.exists("admin_systems.json"):
                        with st.expander("🕵️ View Active ADMIN SECRETS"):
                            try:
                                with open("admin_systems.json", "r") as f: admin_dict = json.load(f)
                                if admin_dict:
