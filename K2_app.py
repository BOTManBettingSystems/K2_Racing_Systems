...
# --- Tab 4: Mini SYSTEM BUILDER ---
    with tab4:
        ...
        if df_all is not None and not df_all.empty:
            ...
            # --- UI STATE INITIALIZATION FOR LOADING SYSTEMS ---
            ...
            defaults = {
                ...
                'ui_courses': [], 'ui_race_types': rt_opts, 'ui_hcap_types': hcap_opts,
            ...
            # --- FUNCTION TO LOAD JSON DATA INTO UI STATE ---
            def load_sys_to_ui(sys_data):
                st.session_state['ui_months'] = sys_data.get('months', ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"])
                ...
                st.session_state['ui_rnrs'] = sys_data.get('rnrs', ["2-7", "8-12", "13-16", ">16"])
            ...
            with st.form("builder_form"):
                ...
                with m_col:
                    ...
                    selected_months = st.multiselect("Include Specific Months (Seasonal Filter)", all_months, key="ui_months")
                
                st.markdown("---")
                selected_courses = st.multiselect("🎯 Specific Course(s) [Leave blank to include ALL courses]", course_opts, key="ui_courses")
                st.markdown("<br>", unsafe_allow_html=True)

                c1, c2, c3, c4 = st.columns(4)
                with c1:
                    selected_race_types = st.multiselect("Race Type", rt_opts, key="ui_race_types")
                    selected_hcap = st.multiselect("Handicap Status", hcap_opts, key="ui_hcap_types")
                with c2:
                    p_col1, p_col2 = st.columns(2)
                    with p_col1: price_min = st.number_input("Min Price", 0.0, 1000.0, step=0.5, key="ui_price_min")
                    with p_col2: price_max = st.number_input("Max Price", 0.0, 1000.0, step=0.5, key="ui_price_max")
                    min_prob_gap = st.number_input("Minimum Prob Gap (%)", -100.0, 50.0, step=0.5, key="ui_prob_gap") / 100
                with c3:
                    selected_rnrs = st.multiselect("No. of Runners", rnr_opts, key="ui_rnrs")
                    if class_opts:
                        selected_classes = st.multiselect("Class (1-6)", class_opts, key="ui_classes")
                    else:
                        st.multiselect("Class (1-6)", ["Not Found in CSV"], disabled=True)
                        selected_classes = []
                with c4:
                    if cm_opts:
                        selected_cm = st.multiselect("Class Movement", cm_opts, key="ui_cm")
                    else:
                ...
                with c5:
                    ai_rank_filter = st.selectbox("AI Rank", ["Any", "AI Rank 1 Only", "Top 2 Only", "NOT AI Rank 1", "NOT Top 3"], key="ui_ai_rank_filter")
                    selected_sex = st.multiselect("Horse Sex", sex_opts, key="ui_sex")
                with c6: 
                ...
                with c7: irish_f = st.selectbox("Irish Race", ["Any", "Y (Yes)", "No (Blank)"], key="ui_irish_f")
                with c8: age_min, age_max = st.slider("Horse Age Range", 1, 20, key="ui_age_range")
                
                with st.expander("📊 Advanced Rank Filters", expanded=False):
                    rank_opts = ["Any", "Rank 1", "Top 2", "Top 3", "Not Top 3"]
                    
                    r1_c1, r1_c2, r1_c3, r1_c4, r1_c5 = st.columns(5)
                    with r1_c1: comb_f = st.selectbox("Comb. Rank", rank_opts, key="ui_comb_f")
                    with r1_c2: comp_f = st.selectbox("Comp. Rank", rank_opts, key="ui_comp_f")
                    with r1_c3: speed_f = st.selectbox("Speed Rank", rank_opts, key="ui_speed_f")
                    with r1_c4: race_f = st.selectbox("Race Rank", rank_opts, key="ui_race_f")
                    with r1_c5: primary_f = st.selectbox("Primary Rank", rank_opts, key="ui_primary_f")
                    
                    r2_c1, r2_c2, r2_c3, r2_c4, r2_c5 = st.columns(5)
                    with r2_c1: msai_f = st.selectbox("MSAI Rank", rank_opts, key="ui_msai_f")
                    with r2_c2: prb_f = st.selectbox("PRB Rank", rank_opts, key="ui_prb_f")
                    with r2_c3: tprb_f = st.selectbox("Trainer PRB Rank", rank_opts, key="ui_tprb_f")
                    with r2_c4: jprb_f = st.selectbox("Jockey PRB Rank", rank_opts, key="ui_jprb_f")
                    with r2_c5: form_f = st.selectbox("Form Rank", rank_opts, key="ui_form_f")
                    
                    r3_c1, r3_c2, r3_c3, r3_c4, r3_c5 = st.columns(5)
                    with r3_c1: pure_f = st.selectbox("Pure Rank", rank_opts, key="ui_pure_f")
